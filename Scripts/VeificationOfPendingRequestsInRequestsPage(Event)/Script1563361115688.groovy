...
def StudentName = CustomKeywords.'com.CommonUtilities.ConfirmationPage.CompleteRegistration'([VisitType], findTestObject('Object Repository/Add Attendee/EventsOptionInAddAttendee'))
...
CustomKeywords.'com.CommonUtilities.Verifications.VerifyTextMatch'(NumberOfRequestsAfterSubmittingARequest, (NumberOfRequests+1).toString(), "After submitting an event request the count on Request left nav is shown incorrect. Expected: "+(NumberOfRequests+1).toString()+", Actual: "+NumberOfRequestsAfterSubmittingARequest)
...
CustomKeywords.'com.CommonUtilities.Verifications.VerifyElementPresent'(findTestObject('Object Repository/Requests/StudentNameInRequestsPage(student)', ["student":StudentName]), "After submitting an event request, the request is not shown in Pending tab of Requests page")
