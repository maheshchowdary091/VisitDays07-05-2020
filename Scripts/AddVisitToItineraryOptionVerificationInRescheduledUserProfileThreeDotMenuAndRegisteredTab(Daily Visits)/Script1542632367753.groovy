...
WebUI.waitForElementVisible(findTestObject('DailyVisitsPage/Registered/SecondAvailableDateInRescheduleModal'), 0)

WebUI.click(findTestObject('DailyVisitsPage/Registered/SecondAvailableDateInRescheduleModal'), FailureHandling.STOP_ON_FAILURE)
...
if (!(WebUI.verifyElementPresent(findTestObject('DailyVisitsPage/AddVisitToItineraryOptionInRegisteredTab'), 0, FailureHandling.OPTIONAL))) {
    KeywordUtil.markFailed('"Add Visit To Itinerary" option is not shown in The Reschedued Group three dot menu in Canceled tab')
...
//(//div[@class="day-wrapper flex-stretch"])[2]//i[text()='more_vert']
def ThredotMenuOfRescheduledStudent = new TestObject('Three dot menu for rescheduled user')

ThredotMenuOfRescheduledStudent.addProperty('xpath', ConditionType.EQUALS, ('//h3[text()="' + VisitType) + '"]/../..//i[text()="more_vert"]')

if (!(WebUI.verifyElementNotPresent(ThredotMenuOfRescheduledStudent, 0, FailureHandling.OPTIONAL))) {
    KeywordUtil.markFailed('Three dot menu is shown for a rescheduled visits in user profile (Expected : Three dot menu should not render for the reschedued visits in user profile)')
