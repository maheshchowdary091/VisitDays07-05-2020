...
if(!(WebUI.verifyElementNotPresent(findTestObject('DailyVisitsPage/DeleteButtonInDeleteDailyVisitsModal'), 0)))
{
...
if (!(WebUI.verifyElementNotPresent(visit, 0, FailureHandling.OPTIONAL))) {
	KeywordUtil.markFailed('After deleting the drafted visit from index page using \'Delete all Draft\' button the drafted visits are not deleted')
