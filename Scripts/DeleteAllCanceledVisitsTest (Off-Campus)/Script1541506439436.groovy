...
not_run: driver.findElement(By.xpath(('//li[text()=\'' + VisitType) + '\']')).click()
...
if (!(WebUI.verifyElementNotPresent(findTestObject('DailyVisitsPage/Yes,IUnderstandCheckBoxInDeleteVisitsModal'), 0, FailureHandling.OPTIONAL))) {
	KeywordUtil.markFailed('After clicking Delete button in the Delete visit modal the modal is not getting disappeared')
...
if (!(WebUI.verifyElementNotPresent(visit, 0, FailureHandling.OPTIONAL))) {
	KeywordUtil.markFailedAndStop('After deleting all Canceled visits from index the canceled visits are still shown in index page')
