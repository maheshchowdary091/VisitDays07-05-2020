...
CustomKeywords.'com.CommonUtilities.CreationOperations.CreateInstantDailyVisitByAssigningUsers'(DailyVisitVisitType)
...
object.addProperty('xpath', ConditionType.EQUALS, ('//strong[text()=\'' + DailyVisitVisitType) + '\']')

WebUI.scrollToElement(object, 0)

WebUI.click(object)

WebUI.verifyElementText(findTestObject('Add Attendee/VisitNameInVisitDescriptionInMobi'), DailyVisitVisitType)
...
if (!(WebUI.verifyElementPresent(findTestObject('Add Attendee/WhoYouWillMeetUserProfile'), 0, FailureHandling.OPTIONAL))) {
	KeywordUtil.markFailed('Who You Will Meet Admin profile is not shown for Daily Visits')
